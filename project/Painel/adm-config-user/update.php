...
if (!empty($_POST)) {
    ...
    if ($valusuario_idacao) {
        ...
        $sql = "UPDATE usuario  set usuario = ?, senha = ?, email = ?, endereco = ?, numero = ?, complemento = ?, placa = ?,  WHERE usuario_id = ?";
        $q = $pdo->prepare($sql);
        $q->execute(array($usuario, $senha, $email, $endereco, $numero, $complemento, $placa, $usuario_id));
        Banco::desconectar();
        header("Location: index.php");
    }
} else {
